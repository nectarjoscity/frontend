      ...
      <div className="max-w-4xl mx-auto flex justify-center">
        <nav className="rounded-full shadow-md border border-green-100 px-3 sm:px-6 py-2 sm:py-3" style={{background: colors.background}}>
          <ul className="flex items-center gap-2 sm:gap-3 md:gap-5 text-xs sm:text-sm md:text-base font-medium" style={{fontFamily: 'Gebuk, Arial, sans-serif', color: colors.text}}>
            <li className="px-2 sm:px-3 md:px-4 py-1 sm:py-2 hover:text-white cursor-pointer hidden sm:block" style={{color: colors.text}}>
              <Link href="/">Menu</Link>
            </li>
            <li 
              className={`px-2 sm:px-3 md:px-4 py-1 sm:py-2 hover:text-white cursor-pointer transition-colors ${pathname === '/contact' ? 'text-white' : ''}`}
              style={{color: pathname === '/contact' ? colors.amber500 : colors.text}}
            >
