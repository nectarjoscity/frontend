...
const masonryStyles = `
  ...
  .add-to-cart-btn:active {
    animation: addToCartPop 0.2s ease-out;
  }
  .add-to-cart-success {
    animation: addToCartPop 0.3s ease-out;
  }
`;
...
function ManualShop({
  ...
  const [addedItems, setAddedItems] = useState(new Set()); // Track recently added items for animation
  const processingRef = useRef(new Set()); // Track items currently being processed

  // Emotional tag configuration
  const emotionalTags = {
    comfort: { emoji: '🏠', label: 'Comfort Classic', color: '#f59e0b' },
    special: { emoji: '⭐', label: 'Special Occasion', color: '#8b5cf6' },
    power: { emoji: '💪', label: 'Power Up', color: '#ef4444' },
    fresh: { emoji: '🌿', label: 'Feel Good', color: '#10b981' },
    share: { emoji: '❤️', label: 'Share the Love', color: '#ec4899' },
  };
  ...
  const handleCategoryClick = useCallback(async (c) => {
    ...
    try {
      ...
      const mapped = Array.isArray(itemsData) ? itemsData.map(it => ({
        ...
        emotionalTag: it.emotionalTag || null,
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 mb-4">
        {categories.map((c) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-semibold transition-all hover:scale-105 ${selectedCategoryId === c._id ? 'text-white' : ''}`}
          ...
          >
            <span className="mr-1">{c.emoji}</span>{c.displayName || c.name}
          </button>
        ))}
      </div>

      {/* Selected Category Header with Tagline */}
      {selectedCategoryId && categories.find(c => c._id === selectedCategoryId) && (
        <div className="mb-6 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold mb-1" style={{ color: colors.text }}>
            {categories.find(c => c._id === selectedCategoryId)?.displayName || categories.find(c => c._id === selectedCategoryId)?.name}
          </h2>
          {categories.find(c => c._id === selectedCategoryId)?.tagline && (
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 flex flex-col grow">
                    {/* Emotional Tag */}
                    {item.emotionalTag && emotionalTags[item.emotionalTag] && (
                      <div
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium mb-2 w-fit"
                        style={{
                          background: `${emotionalTags[item.emotionalTag].color}20`,
                          color: emotionalTags[item.emotionalTag].color
                        }}
                      >
                        <span>{emotionalTags[item.emotionalTag].emoji}</span>
                        <span>{emotionalTags[item.emotionalTag].label}</span>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 mt-auto">
                      ...
                      <button
                        onClick={() => {
                          if (!cart.some(ci => ci.name === item.name) && item.isAvailable !== false) {
                            addToCart(item);
                            // Trigger animation
                        ...
                        className={`w-1/2 font-semibold py-2.5 rounded-lg transition-all add-to-cart-btn ${addedItems.has(item._id) ? 'add-to-cart-success' : ''} ${item.isAvailable === false
